...
The `clients` and `risk_scores` tables enforce four clinical recovery levels:

```
Score: 0 – 29   -->  STABLE      (Green)   - Normal weekly monitoring
Score: 30 – 49  -->  MONITOR     (Yellow)  - Increased attention required
Score: 50 – 74  -->  AT_RISK     (Orange)  - Active risk alert raised
Score: 75 – 100 -->  CRITICAL    (Red)     - Immediate clinical intervention required
```
...
The schema creates indexes on critical query paths:
- `idx_clients_phone` on `clients(phone_number)` for fast SMS webhook lookup.
- `idx_clients_risk_level` on `clients(current_risk_level)` for dashboard KPIs.
- `idx_checkins_client` on `check_ins(client_id)` for timeline retrieval.
- `idx_risk_alerts_status` on `risk_alerts(status)` for active alert badges.
- `idx_notifications_user_unread` on `notifications(user_id, is_read)` for notification polling.
- `idx_audit_created` on `audit_logs(created_at)` for compliance auditing.
